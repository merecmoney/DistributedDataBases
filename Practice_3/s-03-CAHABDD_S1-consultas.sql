...
--La consulta se lanza en cahabdd_s1
set serveroutput on
declare
    v_num_bancos number;
    v_num_paises number;
...
begin
    ...
    select count(*)
        ...
        from (
            select EMPLEADO_ID
            from F_CAH_EMPLEADO_1
            union all
            select EMPLEADO_ID
            from F_CAH_EMPLEADO_2@cahabdd_s2.fi.unam
        ) EMPLEADOS;
    ...
    select count(*)
      ...
      from (
        select c1.CUENTA_ID
        from F_CAH_CUENTA_1 c1
        join F_CAH_CUENTA_2@cahabdd_s2.fi.unam c2
    ...
    select count(*)
        ...
        from (
            ...
            union all
            select NUM_MOVIMIENTO
            from F_CAH_MOVIMIENTO_1@cahabdd_s2.fi.unam
            union all
            select NUM_MOVIMIENTO
            from F_CAH_MOVIMIENTO_3@cahabdd_s2.fi.unam
        ) MOVIMIENTO;
